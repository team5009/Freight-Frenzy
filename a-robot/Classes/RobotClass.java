...
public class RobotClass {
  ...
  public void mainArm(int pos, double power) {
    Arm.setPower(power);
    if(pos == 1) {
      while(Math.abs(Arm.getCurrentPosition()) < 90) {}
    } else if (pos == 2) {
      while(Math.abs(Arm.getCurrentPosition()) < 170) {}
    } else if (pos == 3) {
      while(Math.abs(Arm.getCurrentPosition()) < 260) {}
    }
    Arm.setPower(0);
    resetEncoders();
  }
